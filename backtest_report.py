...
def convert_datatypes(df):
    ...
    df["BT from"] = pd.to_datetime(df["BT from"], errors="coerce")

    df["BT to"] = pd.to_datetime(df["BT to"], errors="coerce")
...
def validate_values(df):
    ...
    if (df["Trades"] <= 0).any():
        logger.warning("Trades <= 0 detected.")

    if (df["Years"] <= 0).any():
        logger.warning("Years <= 0 detected.")

    if (df["Avg days"] <= 0).any():
        logger.warning("Avg Days <= 0 detected.")

    if ((df["Win%"] < 0) | (df["Win%"] > 100)).any():
        logger.warning("Win% outside 0-100.")

    if ((df["Target %"] + df["Trail %"] + df["Stop %"] + df["Time %"]) > 100.5).any():
        logger.warning("Exit Percentages exceed 100%.")

    if (
        (df["Target #"] + df["Trail #"] + df["Stop #"] + df["Time #"]) > df["Trades"]
    ).any():
        logger.warning("Exit Counts exceed Trades.")
...
def calculate_metrics(df):
    ...
    df["Loss %"] = (100 - df["Win%"]).round(CONFIG["ROUND"])
    ...
    df["Reward Risk"] = rr.replace([np.inf, -np.inf], np.nan).clip(upper=10)

    # --------------------------------------------------------
    # 3. PROFIT FACTOR
    # --------------------------------------------------------

    gross_profit = (df["Win%"] / 100) * df["Avg win%"]

    gross_loss = (df["Loss %"] / 100) * df["Avg loss%"]

    pf = safe_divide(gross_profit, gross_loss)

    pf = pf.replace([np.inf, -np.inf], np.nan).clip(upper=20)
    ...
    df["Expected Return"] = (gross_profit - gross_loss).round(CONFIG["ROUND"])

    # --------------------------------------------------------
    # 6. TRADES / YEAR
    # --------------------------------------------------------

    df["Trades / Year"] = safe_divide(df["Trades"], df["Years"]).round(CONFIG["ROUND"])

    # --------------------------------------------------------
    # 7. TRADE CONFIDENCE
    # --------------------------------------------------------

    df["Trade Confidence"] = (df["Trades"] * df["Win%"] / 100).round(CONFIG["ROUND"])

    # --------------------------------------------------------
    # 8. HOLDING EFFICIENCY
    # --------------------------------------------------------

    df["Holding Efficiency"] = safe_divide(df["Expectancy%"], df["Avg days"]).round(
        CONFIG["ROUND"]
    )
    ...
    df["Winning Exit %"] = (df["Target %"] + df["Trail %"] + df["Time-win"]).round(
        CONFIG["ROUND"]
    )
    ...
    df["Losing Exit %"] = (df["Stop %"] + df["Time-loss"]).round(CONFIG["ROUND"])

    # --------------------------------------------------------
    # 11. EXIT EDGE
    # --------------------------------------------------------

    df["Exit Edge"] = (df["Winning Exit %"] - df["Losing Exit %"]).round(
        CONFIG["ROUND"]
    )
    ...
    df["Target Ratio"] = (
        safe_divide(df["Target #"], df["Trades"]).mul(100).round(CONFIG["ROUND"])
    )
    ...
    df["Trail Ratio"] = (
        safe_divide(df["Trail #"], df["Trades"]).mul(100).round(CONFIG["ROUND"])
    )
    ...
    df["Stop Ratio"] = (
        safe_divide(df["Stop #"], df["Trades"]).mul(100).round(CONFIG["ROUND"])
    )
    ...
    df["Time Exit Ratio"] = (
        safe_divide(df["Time #"], df["Trades"]).mul(100).round(CONFIG["ROUND"])
    )
    ...
    df["Opportunity Score"] = safe_divide(
        df["Signals today"] * df["RS%"], np.log1p(df["Rank"])
    ).round(CONFIG["ROUND"])
...
def build_scorecard(df):
    ...
    df["Strategy Rank"] = df.index + 1

    df["Recommendation"] = df["Overall Score"].apply(recommendation)
...
def report_preview(df):
    ...
    print(df[["Strategy Rank", "Stock", "Overall Score", "Recommendation"]].head(20))
