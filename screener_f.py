...
def generate_signals(df: pd.DataFrame, strategy: str, p: dict,
    ...
    # --- OVER-EXTENSION GUARD: skip signals when price has already sprinted so far above
    # its short-term MA that a mean-reversion drop is the more likely next move than
    # continuation. Filters the classic "buy near the top of a parabolic move" trap.
    max_ext_20 = p.get("max_ext_20")           # % above 20-DMA ceiling (e.g. 15)
    max_rsi    = p.get("max_rsi")               # RSI ceiling (e.g. 75)
    if max_ext_20 is not None:
        sig = sig & (df["pct_vs_sma20"].fillna(0) <= max_ext_20)
    if max_rsi is not None:
        sig = sig & (df["rsi14"].fillna(0) <= max_rsi)
...
def run_backtest(df: pd.DataFrame, target_pct: float, max_hold: int,
                 ...
                 trail_anchor: str = "ATR") -> pd.DataFrame:
    ...
    for i in range(n - 1):
        ...
        partial_level = entry + partial_atr * a if (partial_frac > 0 and np.isfinite(a) and a > 0) else np.inf
            ...
            if exit_mode == "Fixed target":
                ...
                if h[d] > peak:
                    ...
                    if trail_anchor == "Structure":
                        ...
                        stop = max(stop, peak - atr_dist)  # trail up under the rising peak
...
def summarize(trades: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "avg_win_%": round(trades.loc[trades["net_return_%"] > 0, "net_return_%"].mean(), 2)
                     if wins else 0.0,
        "avg_loss_%": round(trades.loc[trades["net_return_%"] <= 0, "net_return_%"].mean(), 2)
                      if wins < n else 0.0,
...
def main():
    ...
    with st.sidebar:
        ...
        if exit_mode.startswith("Trailing"):
            ...
            lock_pct = st.slider("Lock profit at (%)", 0.0, 30.0, 7.0, 0.5,
        ...
        with st.expander("Advanced (defaults are HUDCO-derived)"):
            p = {
                ...
                "max_ext_20": st.slider("Skip if price above 20-DMA by more than (%)",
                                        5.0, 40.0, 15.0, 1.0,
                                        help="Over-extension guard: parabolic runs where price sprints "
                                             "well above its 20-DMA typically end in mean-reversion "
                                             "drops. Skips those setups so you don't buy near the top."),
                "max_rsi": st.slider("Skip if RSI(14) above", 60.0, 85.0, 75.0, 1.0,
                                     help="Momentum-exhaustion guard: RSI in the 75-80 zone at entry "
                                          "has poor forward returns even in strong uptrends."),
            }
    ...
    m2[1].metric("Expectancy / trade", f'{stats["expectancy_%"]}%',
                 f'{stats.get("exp_per_day_%", 0)}% per day')
    m2[2].metric("Avg win / avg loss", f'{stats["avg_win_%"]}% / {stats["avg_loss_%"]}%')
