...
class ChartEngine:
    ...
    def ranking_chart(self, top_n=TOP_N):
        if self.df.empty:
            return go.Figure()

        dataframe = self.df.head(top_n).sort_values("Overall Score")
    ...
    def grade_distribution(self):
        ...
        data = self.df["Grade"].value_counts().reset_index()
    ...
    def recommendation_distribution(self):
        ...
        data = self.df["Recommendation"].value_counts().reset_index()

        data.columns = ["Recommendation", "Count"]
    ...
    def radar(self, strategy):
        if self.df.empty:
            return go.Figure()

        row = self.df[self.df["Strategy"] == strategy]
        ...
        labels = [metric.replace("_Mean", "") for metric in metrics]

        fig = go.Figure()

        fig.add_trace(
            go.Scatterpolar(r=values, theta=labels, fill="toself", name=strategy)
        )
    ...
    def multi_radar(self, strategies=None):
        if self.df.empty:
            return go.Figure()
        ...
        if strategies is None:
            strategies = self.df["Strategy"].head(3).tolist()
        ...
        for strategy in strategies:
            ...
            fig.add_trace(
                go.Scatterpolar(
                    ...
                    theta=[metric.replace("_Mean", "") for metric in metrics],
                    fill="toself",
                    name=strategy,
                )
            )
    ...
    def contribution_chart(self, strategy):
        row = self.df[self.df["Strategy"] == strategy]
        ...
        data = pd.DataFrame(
            {
                "Metric": [
                    column.replace("_Contribution", "")
                    for column in contribution_columns
                ],
                "Contribution": [row[column] for column in contribution_columns],
            }
        )
    ...
    def metric_trend(self, metric="Overall Score"):
    ...
    def dashboard(self):
        return {
            ...
            "parallel": self.parallel_coordinates(),
            "metric_comparison": self.metric_comparison(),
            "top_strategies": self.top_strategies(),
            "bottom_strategies": self.bottom_strategies(),
        }
    ...
    def executive_kpi(self):
        ...
        return {
            ...
            "Top Strategy": self.df.iloc[0]["Strategy"],
            "Average Percentile": (
    ...
    def report(self):
        ...
        return {
            "executive_kpi": self.executive_kpi(),
            "overall_score": charts["overall_score"],
            "ranking": charts["ranking"],
            "grade_distribution": charts["grade_distribution"],
            "recommendation_distribution": charts["recommendation_distribution"],
            "scatter": charts["scatter"],
            "bubble": charts["bubble"],
            "histogram": charts["histogram"],
            "boxplot": charts["boxplot"],
            "violin": charts["violin"],
            "correlation": charts["correlation"],
            "parallel": charts["parallel"],
            "metric_comparison": charts["metric_comparison"],
            "top_strategies": charts["top_strategies"],
            "bottom_strategies": charts["bottom_strategies"],
        }
