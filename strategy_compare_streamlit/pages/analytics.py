...
class AnalyticsPage:
    ...
    def kpis(self):
        ...
        best = self.ranked.iloc[0]["Strategy"]
    ...
    def radar_chart(self):
        ...
        strategy = st.selectbox(
            "Select Strategy", self.ranked["Strategy"], key="analytics_radar"
        )

        figure = self.charts.radar(strategy)
    ...
    def multi_radar_chart(self):
        st.subheader("Multi Strategy Radar")

        selected = st.multiselect(
            "Compare Strategies",
            self.ranked["Strategy"],
            default=list(self.ranked["Strategy"].head(2)),
            key="analytics_multi_radar",
        )

        if len(selected) < 2:
            st.info("Select at least two strategies.")

            return

        figure = self.charts.multi_radar(selected)
    ...
    def scatter_chart(self):
        st.subheader("Performance vs Reliability")
    ...
    def violin_plot(self):
        st.subheader("Overall Score Density")
    ...
    def parallel_coordinates(self):
        ...
        if figure is not None:
            ...
            st.info("Parallel coordinates unavailable.")
    ...
    def metric_comparison(self):
        ...
        if figure is not None:
            ...
            st.info("Metric comparison chart unavailable.")

    ###########################################################################
    # TOP STRATEGIES
    ###########################################################################

    def top_strategies(self):
        st.subheader("Top Strategies")

        figure = self.charts.top_strategies(10)

        if figure is not None:
            st.plotly_chart(figure, width="stretch")

        else:
            st.info("Top strategies chart unavailable.")

    ###########################################################################
    # BOTTOM STRATEGIES
    ###########################################################################

    def bottom_strategies(self):
        st.subheader("Bottom Strategies")

        figure = self.charts.bottom_strategies(10)

        if figure is not None:
            st.plotly_chart(figure, width="stretch")

        else:
            st.info("Bottom strategies chart unavailable.")
        ...
        summary = pd.DataFrame(
            {
                ...
                "Value": [
                    ...
                    round(self.ranked["Overall Score"].min(), 2),
