...
class InsightEngine:
    ...
    def executive_summary(self):
        ...
        rows.append({"Insight": "Best Strategy", "Value": best["Strategy"]})

        rows.append(
            {"Insight": "Best Overall Score", "Value": round(best["Overall Score"], 2)}
        )

        rows.append({"Insight": "Recommendation", "Value": best["Recommendation"]})

        rows.append({"Insight": "Lowest Ranked Strategy", "Value": worst["Strategy"]})

        rows.append(
            {"Insight": "Lowest Score", "Value": round(worst["Overall Score"], 2)}
        )

        rows.append({"Insight": "Strategies Compared", "Value": len(self.df)})
            ...
            rows.append(
                {
                    ...
                    "Value": round(self.df.loc[idx, metric], 2),
                }
            )
    ...
    def deployment(self):
        ...
        for _, row in self.df.iterrows():
            ...
            rows.append(
                {"Strategy": row["Strategy"], "Decision": decision, "Reason": reason}
            )
