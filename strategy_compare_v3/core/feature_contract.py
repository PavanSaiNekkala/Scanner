...
class FeatureContract:
    ...
    def normalize(cls, dataframe: pd.DataFrame) -> pd.DataFrame:
        ...
        if created:
            logger.info("Feature contract created columns: %s", sorted(set(created)))

        else:
            logger.info("Feature contract: no normalization required.")
    ...
    @classmethod
    def validate(cls, dataframe: pd.DataFrame, required: list[str]):
...
if __name__ == "__main__":
    print("Import FeatureContract into engines")
