...
class EfficiencyMetricsEngine:
    ...
    def return_per_holding_day(self):
        required = {"Avg win%", "Avg days"}

        if not required.issubset(self.df.columns):
            return

        self.df["Return Per Holding Day"] = self.safe_divide(
            self.df["Avg win%"], self.df["Avg days"]
        )
    ...
    def profit_velocity(self):
        ...
        self.df["Profit Velocity"] = (
            self.safe_divide(self.df["Expectancy%"], self.df["Avg days"]) * 365
        )
    ...
    def trade_efficiency(self):
        ...
        self.df["Trade Efficiency"] = self.safe_divide(
    ...
    def capital_efficiency(self):
        ...
        risk = self.df["Avg loss%"].abs()

        self.df["Capital Efficiency"] = self.safe_divide(self.df["Expectancy%"], risk)
    ...
    def time_efficiency(self):
        required = {"Avg days", "Years"}

        if not required.issubset(self.df.columns):
            return

        self.df["Time Efficiency"] = self.safe_divide(
            self.df["Years"], self.df["Avg days"]
        )
    ...
    def annual_capital_productivity(self):
        ...
        annual_trades = self.safe_divide(self.df["Trades"], self.df["Years"])

        self.df["Annual Capital Productivity"] = self.df["Expectancy%"] * annual_trades
    ...
    def strategy_efficiency_index(self):
        required = {"Holding Efficiency", "Capital Efficiency", "Trade Efficiency"}
    ...
    def generate(self):
        logger.info("Generating Efficiency Metrics...")
...
if __name__ == "__main__":
    print("Import EfficiencyMetricsEngine")
