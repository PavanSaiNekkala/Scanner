...
class OpportunityMetricsEngine:
    ...
    def signal_strength(self):
        ...
        self.df["Signal Strength"] = np.sqrt(self.df["Signals today"])
    ...
    def rank_quality(self):
        ...
        self.df["Rank Quality"] = 1 / np.log1p(self.df["Rank"])
    ...
    def rs_impact(self):
        ...
        self.df["RS Impact"] = self.df["RS%"] / 100
    ...
    def opportunity_index(self):
        required = {"Signals today", "RS%", "Rank"}

        if not required.issubset(self.df.columns):
            return

        self.df["Opportunity Index"] = (
            np.sqrt(self.df["Signals today"])
            * (self.df["RS%"] / 100)
            / np.log1p(self.df["Rank"])
        )
    ...
    def opportunity_density(self):
        required = {"Signals today", "Trades"}

        if not required.issubset(self.df.columns):
            return

        self.df["Opportunity Density"] = self.safe_divide(
    ...
    def opportunity_quality(self):
        required = {"Opportunity Index", "Win%"}

        if not required.issubset(self.df.columns):
            return

        self.df["Opportunity Quality"] = self.df["Opportunity Index"] * (
            self.df["Win%"] / 100
        )
    ...
    def market_opportunity_factor(self):
        required = {"Opportunity Index", "Expectancy%"}

        if not required.issubset(self.df.columns):
            return

        self.df["Market Opportunity Factor"] = (
            self.df["Opportunity Index"] * self.df["Expectancy%"]
        )
    ...
    def generate(self):
        logger.info("Generating Opportunity Metrics...")
...
if __name__ == "__main__":
    print("Import OpportunityMetricsEngine")
