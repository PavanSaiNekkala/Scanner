...
class StatisticalMetricsEngine:
    ...
    def coefficient_variation(self, statistics_df):
        statistics_df["Coefficient Variation"] = statistics_df[
            "Std Dev"
        ] / statistics_df["Mean"].replace(0, np.nan)
    ...
    def trade_distribution_quality(self):
        if "net_return_Pct" not in self.df.columns:
            return pd.DataFrame()

        returns = self.df["net_return_Pct"].dropna()

        result = {
            "Winning Trades": (returns > 0).sum(),
            "Losing Trades": (returns < 0).sum(),
    ...
    def generate(self):
        ...
        statistics = self.generate_basic_statistics()

        statistics = self.coefficient_variation(statistics)

        percentiles = self.percentile_analysis()

        distribution = self.trade_distribution_quality()

        logger.info("Statistical Metrics completed.")
...
if __name__ == "__main__":
    print("Import StatisticalMetricsEngine")
