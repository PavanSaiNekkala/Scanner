...
class EfficiencyEngine:
    ...
    def profit_per_day(self):
        required = {"Profit Factor", "Avg days"}

        if not required.issubset(self.df.columns):
            return

        self.df["Profit Per Day"] = self.safe_divide(
            self.df["Profit Factor"], self.df["Avg days"]
        )

    # -----------------------------------------------------

    def signal_efficiency(self):
        required = {"Signals today", "Win %"}

        if not required.issubset(self.df.columns):
            return

        self.df["Signal Efficiency"] = self.safe_divide(
            self.df["Win %"], self.df["Signals today"]
        )
    ...
    def trade_efficiency(self):
        ...
        self.df["Trades Per Day"] = 1 / self.df["days_held"].replace(0, np.nan)

    def holding_efficiency(self):
        required = {"net_return_Pct", "days_held"}

        if not required.issubset(self.df.columns):
            return

        self.df["Holding Efficiency"] = self.df["net_return_Pct"] / self.df[
            "days_held"
        ].replace(0, np.nan)

    def capital_efficiency(self):
        required = {"net_return_Pct", "entry_price"}

        if not required.issubset(self.df.columns):
            return

        self.df["Capital Efficiency"] = (
            self.df["net_return_Pct"] / self.df["entry_price"]
        )

    # -----------------------------------------------------

    def holding_efficiency(self):
        required = {"Win %", "Avg days"}

        if not required.issubset(self.df.columns):
            return

        self.df["Holding Efficiency"] = self.safe_divide(
            self.df["Win %"], self.df["Avg days"]
        )
    ...
    def efficiency_score(self):
        required = {"Expectancy Per Trade", "Profit Per Day", "Holding Efficiency"}
    ...
    def efficiency_grade(self):
        ...
        conditions = [score >= 80, score >= 60, score >= 40, score >= 20]

        choices = ["Excellent", "Good", "Average", "Below Average"]

        self.df["Efficiency Grade"] = np.select(conditions, choices, default="Poor")

    # -----------------------------------------------------

    def generate(self):
        logger.info("Generating Efficiency Features...")
        ...
        logger.info("Efficiency feature engineering completed.")
