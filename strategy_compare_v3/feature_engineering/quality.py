...
class QualityEngine:
    ...
    def trade_quality(self):
        ...
        self.df["Trade Quality"] = self.safe_divide(self.df["Win %"], self.df["Trades"])
    ...
    def expectancy_quality(self):
        required = {"Expectancy%", "Reward Risk Ratio"}

        if not required.issubset(self.df.columns):
            return

        self.df["Expectancy Quality"] = (
            self.df["Expectancy%"] * self.df["Reward Risk Ratio"]
        )
    ...
    def execution_quality(self):
        ...
        self.df["Execution Quality"] = (
            self.df["Holding Efficiency"] * self.df["Edge Ratio"]
        )

    # -----------------------------------------------------

    def signal_quality(self):
        required = {"Trades", "Profit Factor"}
    ...
    def generate(self):
        logger.info("Generating Quality Features...")
        ...
        logger.info("Quality feature engineering completed.")
