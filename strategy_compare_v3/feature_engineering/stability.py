...
class StabilityEngine:
    ...
    def trade_density(self):
        if "days_held" in self.df.columns:
            self.df["Trade Density"] = 1 / self.df["days_held"].replace(0, np.nan)

            return

        required = {"Trades", "Avg days"}
    ...
    def profit_consistency(self):
        required = {"Profit Factor", "Reward Risk Ratio"}
    ...
    def expectancy_stability(self):
        if {"Expectancy%", "days_held"}.issubset(self.df.columns):
        ...
        if {"Expectancy%", "Avg days"}.issubset(self.df.columns):
            self.df["Expectancy Stability"] = self.safe_divide(
    ...
    def reward_consistency(self):
        required = {"Reward Risk Ratio", "Profit Factor"}

        if not required.issubset(self.df.columns):
            return

        self.df["Reward Consistency"] = self.safe_divide(
            self.df["Reward Risk Ratio"], self.df["Profit Factor"]
        )
    ...
    def holding_stability(self):
        if {"days_held", "Trades"}.issubset(self.df.columns):
            self.df["Holding Stability"] = self.safe_divide(
                self.df["days_held"], self.df["Trades"]
            )

            return

        if {"Avg days", "Trades"}.issubset(self.df.columns):
            self.df["Holding Stability"] = self.safe_divide(
    ...
    def robustness_index(self):
        if {"Stability Index", "Profit Factor"}.issubset(self.df.columns):
            self.df["Robustness Index"] = (
                self.df["Stability Index"] * self.df["Profit Factor"]
            )
    ...
    def consistency_score(self):
        if {"Robustness Index", "Expectancy%"}.issubset(self.df.columns):
            self.df["Consistency Score"] = (
                self.df["Robustness Index"] * self.df["Expectancy%"]
            )
    ...
    def generate(self):
        logger.info("Generating Stability Features...")
        ...
        logger.info("Stability feature engineering completed.")

        return self.df


if __name__ == "__main__":
    print("Import StabilityEngine inside feature_engine.py")
