...
class EfficiencyScoreEngine:
    ...
    def normalize_columns(self):
        mapping = {
            ...
            "Expectancy_Per_Day": "Expectancy Per Day",
        }
    ...
    def generate_missing_features(self):
        ...
        if "Trades Per Day" not in self.df.columns:
            if "Trades" in self.df.columns and "Avg days" in self.df.columns:
                self.df["Trades Per Day"] = self.df["Trades"] / self.df[
                    "Avg days"
                ].replace(0, np.nan)
    ...
    def generate(self):
        logger.info("Generating Efficiency Score...")
        ...
        for feature, weight in self.weights.items():
            if feature not in self.df.columns:
                logger.warning("Skipping missing efficiency feature: %s", feature)
        ...
        if total_weight == 0:
            ...
            self.df["Efficiency Score"] = (score / total_weight).round(2)

        logger.info("Efficiency Score generated.")

        return self.df[["Efficiency Score"]]


if __name__ == "__main__":
    print("Import EfficiencyScoreEngine inside scoring_engine.py")
