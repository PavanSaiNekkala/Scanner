...
def test_single_row():
    df = pd.DataFrame({"Stock": ["ABC"], "Expectancy%": [12], "Profit Factor": [2.1]})
...
def test_duplicate_rows(sample_dataframe):
    duplicated = pd.concat([sample_dataframe, sample_dataframe], ignore_index=True)

    profiler = DataProfiler(duplicated)
