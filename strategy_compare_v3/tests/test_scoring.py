...
def test_score_columns_exist(sample_dataframe):
    ...
    found = [column for column in expected if column in result.columns]
...
def test_score_numeric(sample_dataframe):
    ...
    score_columns = [column for column in result.columns if column.endswith("Score")]
...
def test_empty_dataframe():
    engine = ScoringEngine(pd.DataFrame())
