...
class ScatterPlotVisualizer:
    ...
    def generate_plot(self, x: str, y: str):
        ...
        logger.info("Generating scatter plot %s vs %s", x, y)

        fig, ax = plt.subplots(figsize=(8, 6))

        ax.scatter(self.df[x], self.df[y], alpha=0.70)
        ...
        filename = (
            f"{x}_vs_{y}".replace("/", "_").replace("%", "Pct").replace(" ", "_")
            + ".png"
        )
    ...
    def correlation_table(self):
        ...
        for x, y in self.DEFAULT_PAIRS:
            ...
            rows.append({"X": x, "Y": y, "Correlation": round(corr, 4)})
    ...
    def generate_all(self):
        ...
        for x, y in self.DEFAULT_PAIRS:
            result = self.generate_plot(x, y)

            if result:
                files[f"{x} vs {y}"] = result

        logger.info("%d scatter plots generated.", len(files))
    ...
    def generate(self):
        return {"Files": self.generate_all(), "Correlations": self.correlation_table()}
