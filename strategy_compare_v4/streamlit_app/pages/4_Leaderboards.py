"""
Leaderboards Dashboard
======================

Institutional Leaderboards
"""

from __future__ import annotations

import pandas as pd
import streamlit as st
from components.charts import (
    bar_chart,
    dataframe,
)
from services.loader import get_sheet
from themes import apply_theme


# ============================================================
# Page Configuration
# ============================================================

st.set_page_config(
    page_title="Leaderboards",
    page_icon="🏆",
    layout="wide",
)

apply_theme()


# ============================================================
# Constants
# ============================================================

PAGE_TITLE = "🏆 Institutional Leaderboards"

PAGE_CAPTION = (
    "Overall institutional rankings across strategies and stocks."
)


LEADERBOARD_SHEETS = {
    "overall": "Overall",
    "stocks": "Stocks",
    "strategies": "Strategies",
    "expectancy": "Weighted Expectancy",
    "profit_factor": "Profit Factor",
    "edge": "Edge",
    "reliability": "Reliability",
    "efficiency": "Efficiency",
    "opportunity": "Opportunity",
    "risk": "Risk",
}

TOP_ROWS = 25


# ============================================================
# Header
# ============================================================


def render_header() -> None:
    """
    Render dashboard header.
    """

    st.title(
        PAGE_TITLE,
    )

    st.caption(
        PAGE_CAPTION,
    )

    st.divider()


# ============================================================
# Validation
# ============================================================


def validate_session() -> None:
    """
    Validate report loading status.
    """

    if not st.session_state.get(
        "reports_loaded",
        False,
    ):

        st.warning(
            "Please load reports from the Data Load page.",
        )

        st.stop()


# ============================================================
# Data Loading
# ============================================================


@st.cache_data(
    show_spinner=False,
)
def load_leaderboard_data() -> dict[str, pd.DataFrame]:
    """
    Load leaderboard worksheets.
    """

    workbook = st.session_state.leaderboard_report

    return {
        key: get_sheet(
            workbook,
            sheet_name,
        )
        for key, sheet_name in LEADERBOARD_SHEETS.items()
    }


# ============================================================
# Validation
# ============================================================


def validate_leaderboard_data(
    data: dict[str, pd.DataFrame],
) -> None:
    """
    Validate leaderboard datasets.
    """

    if not data:

        st.error(
            "Leaderboard report unavailable.",
        )

        st.stop()


    if not any(
        not df.empty
        for df in data.values()
    ):

        st.error(
            "No leaderboard data found.",
        )

        st.stop()


# ============================================================
# Column Resolver
# ============================================================


def resolve_column(
    df: pd.DataFrame,
    candidates: list[str],
) -> str | None:
    """
    Resolve available dataframe column.
    """

    for column in candidates:

        if column in df.columns:

            return column


    return None


# ============================================================
# Generic Leaderboard Renderer
# ============================================================


def render_leaderboard(
    title: str,
    df: pd.DataFrame,
) -> None:
    """
    Render leaderboard table and chart.
    """

    st.subheader(
        title,
    )


    if df.empty:

        st.info(
            f"No {title} data available.",
        )

        return


    dataframe(
        df,
    )


    x_col = resolve_column(
        df,
        [
            "Strategy",
            "Stock",
        ],
    )


    y_col = resolve_column(
        df,
        [
            "Composite Score",
            "Institutional Score",
            "Composite",
            "Edge Score",
            "Weighted Expectancy",
            "Profit Factor",
            "Reliability",
            "Efficiency",
            "Opportunity",
            "Risk",
        ],
    )


    if x_col and y_col:

        chart_df = (
            df
            .sort_values(
                y_col,
                ascending=False,
            )
            .head(
                TOP_ROWS,
            )
        )


        bar_chart(
            chart_df,
            x=x_col,
            y=y_col,
            color=x_col,
            title=f"Top {title}",
        )

# ============================================================
# KPI Summary
# ============================================================


def render_summary(
    data: dict[str, pd.DataFrame],
) -> None:
    """
    Render leaderboard summary cards.
    """

    total = len(data)

    available = sum(
        not df.empty
        for df in data.values()
    )

    rows = sum(
        len(df)
        for df in data.values()
    )


    c1, c2, c3 = st.columns(3)


    with c1:

        st.metric(
            "Leaderboard Types",
            total,
        )


    with c2:

        st.metric(
            "Available Leaderboards",
            available,
        )


    with c3:

        st.metric(
            "Total Ranked Records",
            f"{rows:,}",
        )


    st.divider()


# ============================================================
# Download
# ============================================================


def render_download(
    df: pd.DataFrame,
    name: str,
) -> None:
    """
    Render CSV download.
    """

    if df.empty:

        return


    st.divider()

    st.subheader(
        "Export",
    )


    csv = df.to_csv(
        index=False,
    )


    st.download_button(
        label="📥 Download Leaderboard",
        data=csv,
        file_name=f"{name.lower()}_leaderboard.csv",
        mime="text/csv",
        use_container_width=True,
    )


# ============================================================
# Empty State
# ============================================================


def render_empty_state() -> None:
    """
    Render empty state.
    """

    st.info(
        """
### 🏆 Institutional Leaderboards

Load generated reports from the Data Load page.

Available analysis:

- Overall Rankings
- Strategy Rankings
- Stock Rankings
- Expectancy
- Profit Factor
- Edge
- Reliability
- Efficiency
- Opportunity
- Risk
"""
    )


# ============================================================
# Footer
# ============================================================


def render_footer() -> None:
    """
    Render footer.
    """

    st.divider()


    left, right = st.columns(
        [3, 1],
    )


    with left:

        st.caption(
            "Institutional Strategy Comparison Platform",
        )


    with right:

        st.caption(
            "Version 4.0",
        )


# ============================================================
# Main
# ============================================================


def main() -> None:
    """
    Render Leaderboards dashboard.
    """

    render_header()


    validate_session()


    leaderboard_data = load_leaderboard_data()


    validate_leaderboard_data(
        leaderboard_data,
    )


    if not any(
        not df.empty
        for df in leaderboard_data.values()
    ):

        render_empty_state()

        render_footer()

        return


    # --------------------------------------------------------
    # Summary
    # --------------------------------------------------------

    render_summary(
        leaderboard_data,
    )


    # --------------------------------------------------------
    # Leaderboard Selector
    # --------------------------------------------------------

    selected = st.selectbox(
        "Select Leaderboard",
        options=list(
            LEADERBOARD_SHEETS.keys()
        ),
        format_func=lambda x: x.replace(
            "_",
            " ",
        ).title(),
    )


    selected_df = leaderboard_data[selected]


    render_leaderboard(
        selected.replace(
            "_",
            " ",
        ).title(),
        selected_df,
    )


    render_download(
        selected_df,
        selected,
    )


    render_footer()


# ============================================================
# Entry Point
# ============================================================

if __name__ == "__main__":

    main()